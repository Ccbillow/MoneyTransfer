...
//                                                doTransfer(request))))));
    ...
    public void doTransfer(TransferRequest request) {
        ...
        // 1. check user
        checkUser(request.getFromId(), request.getToId(), traceId);

        // 2. get account
        List<Account> accounts = accountRepository.findAllById(Arrays.asList(request.getFromId(), request.getToId()));
     ...
     */
    private void checkUser(Long fromId, Long toId, String traceId) {
        ...
        List<User> users = userClient.getUsers(Arrays.asList(fromId, toId));
